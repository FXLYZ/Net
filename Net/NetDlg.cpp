...
void CNetDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT2, Cmd);
	DDX_Control(pDX, IDC_EDIT4, Result);
}
...
BEGIN_MESSAGE_MAP(CNetDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT1, &CNetDlg::OnEnChangeEdit1)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
BOOL CNetDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	ServerIP.SetWindowTextW(L"127.0.0.1");
	ServerPort.SetWindowTextW(L"2001");
...
{
	// TODO: 在此添加控件通知处理程序代码
	BOOL bflag = mysocket.Create(2000, SOCK_DGRAM, FD_READ|FD_WRITE);
	if (!bflag)
	{
		Result.SetWindowTextW(L"socket创建失败");
	}
	else
	{
		ServerIP.GetWindowTextW(sIP);
		ServerPort.GetWindowTextW(sPORT);
		Cmd.GetWindowTextW(sCMD);
		USES_CONVERSION;
		char* buf = T2A(sCMD);
...
void CNetDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	char* buf = new char[100];
	UINT uport = UINT(_ttoi(sPORT));
	int isR=mysocket.ReceiveFrom(buf,100, sIP, uport, 0);
	if (isR!=-1)
	{
		sResult.Format(L"%s", buf);
		Result.SetWindowTextW(sResult);
		KillTimer(1);
		mysocket.Close();
	}
